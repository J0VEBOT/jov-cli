...
export async function installDaemon(port) {
  const os = platform();
  if (os === 'darwin') await installLaunchd(port);
  else if (os === 'linux') await installSystemd(port);
  else throw new Error(`Daemon not supported on ${os}. Use: jovebot gateway`);
}

async function installLaunchd(port) {
...
<plist version="1.0"><dict>
  ...
  <key>ProgramArguments</key><array><string>${jovPath}</string><string>gateway</string><string>--port</string><string>${port}</string></array>
